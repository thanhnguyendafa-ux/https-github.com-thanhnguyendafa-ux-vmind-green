...
const ReadingScreen: React.FC = () => {
  const { notes, tables, setCurrentScreen, handleCreateNote, handleUpdateNote, handleDeleteNote, handleUpdateTable, showToast, handleSaveToJournal } = useAppContext();
  ...
  const handleUpdateContent = (content: string) => { if (activeNote) { handleUpdateNote({ ...activeNote, content }); } };
  ...
  const handleAddToTable = () => {
    ...
    const newRow: VocabRow = { id: Date.now().toString(), cols: { [selectedColumnId]: selectedText }, stats: { correct: 0, incorrect: 0, lastStudied: null, flashcardStatus: FlashcardStatus.New, flashcardEncounters: 0, isFlashcardReviewed: false, lastPracticeDate: null }, };
    handleUpdateTable({ ...tableToUpdate, rows: [...tableToUpdate.rows, newRow] });
  ...
  const SidebarContent = (
    <>
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center flex-shrink-0">
            ...
            <button onClick={handleCreateNote} className="text-slate-500 dark:text-slate-400 hover:text-emerald-500 transition-colors p-1"><Icon name="plus" className="w-6 h-6"/></button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 p-4 sm:p-6 md:p-10 flex flex-col overflow-y-auto">
            ...
            <>
                ...
                <input type="text" value={activeNote.title} onChange={(e) => handleUpdateNote({...activeNote, title: e.target.value})} className="text-3xl font-bold bg-transparent focus:outline-none focus:bg-white dark:focus:bg-slate-800 w-full rounded-md p-1 -m-1 text-slate-800 dark:text-white" />
                <button onClick={() => handleDeleteNote(activeNote.id)} className="text-slate-400 hover:text-red-500 p-2"><Icon name="trash" className="w-5 h-5"/></button>
                </div>

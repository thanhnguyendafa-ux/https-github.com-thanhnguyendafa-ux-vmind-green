    ...
    <div className="bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700/50 rounded-xl shadow-md transition-all duration-300">
      ...
      {isExpanded && (
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 animate-fadeIn">
          <textarea ref={textareaRef} value={content} onChange={handleContentChange} className="w-full bg-slate-100/50 dark:bg-slate-900/50 rounded-md p-3 text-slate-600 dark:text-slate-300 text-sm leading-relaxed focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none overflow-hidden" placeholder="Start writing..." />
          <div className="mt-4 flex justify-end gap-2">
            <button onClick={handlePrint} className="flex items-center gap-2 px-3 py-1.5 border rounded-md text-sm transition-colors bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"><Icon name="printer" className="w-4 h-4" /> Print</button>
            <button onClick={handleExport} className="flex items-center gap-2 px-3 py-1.5 border rounded-md text-sm transition-colors bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"><Icon name="arrow-down-tray" className="w-4 h-4" /> Export</button>
          </div>
        </div>
      )}
    </div>
...
const JournalScreen: React.FC = () => {
  const { notes, handleUpdateNote, setCurrentScreen } = useAppContext();
      ...
      <main className="space-y-4">
        {journalEntries.length > 0 ? (
          journalEntries.map((note, index) => (
            <JournalEntry key={note.id} note={note} onUpdateNote={handleUpdateNote} isInitiallyExpanded={index === 0} />
          ))
        ) : (
          <div className="text-center py-16">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-2">Entries are automatically added when you save items from study sessions.</p>
          </div>
        )}
      </main>
    </div>

...
import { FlashcardSession, FlashcardStatus, Table, VocabRow, Relation, RelationDesign, TypographyDesign, CardFaceDesign, Theme } from '../types';
...
const FlashcardSessionScreen: React.FC = () => {
  const { activeFlashcardSession, tables, theme, handleFinishFlashcardSession, handleUpdateRow, handleSaveToJournal, handleOpenGalleryView } = useAppContext();
  
  // AppContent logic ensures this component is only rendered when activeFlashcardSession is not null.
  ...
  useEffect(() => { if (!audioCtx.current) { audioCtx.current = new (window.AudioContext || (window as any).webkitAudioContext)({ sampleRate: 24000 }); } }, []);
  ...
  const upgradedRelation = useMemo(() => { if (!currentRelation) return null; return upgradeRelationDesign(currentRelation, theme); }, [currentRelation, theme]);
  ...
  useEffect(() => { setIsFlipped(false); }, [currentSession.currentIndex]);
  ...
  const handleRate = (status: FlashcardStatus) => {
    ...
    if (multiplier === 0) { insertIndex = 1; } else { insertIndex = Math.round(remainingQueue.length * multiplier); }
    ...
    setCurrentSession(prev => ({ ...prev, queue: workQueue, currentIndex: nextIndex >= workQueue.length ? workQueue.length - 1 : nextIndex, sessionEncounters: prev.sessionEncounters + 1, history: [...prev.history, { rowId: currentRowId, status, timestamp: Date.now() }], }));
  };
  ...
  const handlePlayAudio = async (textToSpeak: string) => {
    ...
    } catch (error) { console.error("Error playing audio:", error); setAudioState('error'); setTimeout(() => setAudioState(null), 2000); }
  };
  ...
  if (!currentRow || !upgradedRelation || !currentTable) {
    return (
        <div className="fixed inset-0 bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center">
            ...
            <button onClick={() => handleFinishFlashcardSession(currentSession)} className="bg-emerald-600 text-white font-bold py-3 px-6 rounded-lg">
                Return to Menu
            </button>
        </div>
    );
    ...
    <div className="fixed inset-0 bg-slate-100 dark:bg-slate-900 flex flex-col items-center p-4 transition-colors duration-300">
      <header className="w-full max-w-2xl mb-4">
          <div className="flex justify-between items-center text-slate-500 dark:text-slate-400 mb-2">
            ...
            <button onClick={() => handleFinishFlashcardSession(currentSession)} className="hover:text-slate-800 dark:hover:text-white transition-colors">End Session</button>
          </div>
            ...
            <div className="card-container w-full h-full perspective-1000">
                <div className={`card-flip relative w-full h-full transform-style-3d ${isFlipped ? 'flipped' : ''}`}>
                    <div className="card-front absolute w-full h-full flex flex-col items-center justify-center p-4 rounded-lg shadow-lg border dark:border-slate-700" style={getCardFaceStyle(upgradedRelation.design.front)}>
                        <div className={`flex-1 flex items-center justify-center w-full ${upgradedRelation.design.front.layout === 'vertical' ? 'flex-col gap-2' : 'flex-row gap-4'}`}>
                            {upgradedRelation.questionColumnIds.map(id => { const typography = upgradedRelation.design.front.typography[id] || defaultTypo; const text = currentRow.cols[id] || `[${currentTable.columns.find(c => c.id === id)?.name}]`; return <div key={id} style={{...typography}} className="p-1 break-words">{text}</div>; })}
                        </div>
                    ...
                    <div className="card-back absolute w-full h-full flex flex-col items-center justify-center p-4 rounded-lg shadow-lg border dark:border-slate-700" style={getCardFaceStyle(upgradedRelation.design.back)}>
                        <div className={`flex-1 flex items-center justify-center w-full ${upgradedRelation.design.back.layout === 'vertical' ? 'flex-col gap-2' : 'flex-row gap-4'}`}>
                           {upgradedRelation.answerColumnIds.map(id => { const typography = upgradedRelation.design.back.typography[id] || defaultTypo; const text = currentRow.cols[id] || `[${currentTable.columns.find(c => c.id === id)?.name}]`; return <div key={id} style={{...typography}} className="p-1 break-words">{text}</div>; })}
                        </div>
                        {canPlayBackAudio && <button onClick={(e) => { const audioText = upgradedRelation.answerColumnIds.map(id => currentRow.cols[id]).filter(Boolean).join(' / '); e.stopPropagation(); handlePlayAudio(audioText) }} className="absolute bottom-4 right-4 text-slate-400 hover:text-emerald-500"><Icon name="play" className="w-6 h-6"/></button>}
                    </div>
                </div>
            </div>
        </div>
        {isFlipped && ( <div className="grid grid-cols-5 gap-2 sm:gap-4 w-full animate-fadeIn"> {[FlashcardStatus.Again, FlashcardStatus.Hard, FlashcardStatus.Good, FlashcardStatus.Easy, FlashcardStatus.Perfect].map(status => ( <button key={status} onClick={() => handleRate(status)} className={`py-2 sm:py-3 rounded-lg text-white text-xs sm:text-sm font-bold transition-transform hover:scale-105 ${statusConfig[status].color}`}> {statusConfig[status].label} </button> ))} </div> )}
      </main>
      <footer className="w-full max-w-2xl mt-4 flex items-center justify-between">
          <div className="flex gap-2">
            ...
            <button onClick={() => handleOpenGalleryView(currentTable, currentRow.id)} title="View in Gallery" className="p-2 rounded-full bg-white dark:bg-slate-700 text-slate-500 hover:text-emerald-500 shadow-sm"><Icon name="list-bullet" className="w-5 h-5"/></button>
            <button onClick={() => { const frontText = upgradedRelation.questionColumnIds.map(id => currentRow.cols[id]).filter(Boolean).join(' / '); const backText = upgradedRelation.answerColumnIds.map(id => currentRow.cols[id]).filter(Boolean).join(' / '); handleSaveToJournal("Flashcard Review", `*Q: ${frontText}*\n*A: ${backText}*`); }} title="Save to Journal" className="p-2 rounded-full bg-white dark:bg-slate-700 text-slate-500 hover:text-emerald-500 shadow-sm"><Icon name="book" className="w-5 h-5"/></button>
          </div>
          <div className="text-xs text-slate-500 dark:text-slate-400"> Session Time: {Math.floor((Date.now() - currentSession.startTime) / 60000)}m </div>
      </footer>
       <WordDetailModal isOpen={!!rowForDetail} row={rowForDetail} columns={currentTable.columns} imageColumnId={currentTable.imageConfig?.imageColumnId} onClose={() => setRowForDetail(null)} onSave={(updatedRow) => { handleUpdateRow(updatedRow); setRowForDetail(null); }} onDelete={() => {}} onShowToast={() => {}} onConfigureAI={() => {}} />
    </div>
  );
};
